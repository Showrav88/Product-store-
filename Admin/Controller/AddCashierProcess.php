<?php
include ("../Model/CashierCrude.php");
$fname=$lname=$age=$gender=$contNo=$email=$cid=$pass=$conPass="";
$fnameerr=$lnameerr=$ageerr=$gendererr=$contNoerr=$emailerr=$fileerr=$ciderr=$passerr=$conPasserr=$dataerr="";
$count=$count2=0;
...
if(move_uploaded_file($_FILES["myfile"]["tmp_name"], "../Images/".$fname.$lname."Cashier".date("Y-m-d").".jpg")){
    $count++;
...
if($count==9){
    ...
    $mydb=new DB();
    $conobj=$mydb->opencon();
   $dataerr= $mydb->InsertData($fname,$lname,$age, $_POST["gender"],$contNo,$email,$pass,"cashierinfo",$conobj);
